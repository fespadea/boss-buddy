...
if(is_player_on(bossPlayer)){
	...
	for(var i = 0; i < array_length(bossPlayerID); i++){
		with owner{
			...
			var healthbarHitbox = create_hitbox(AT_HEALTH_BAR, AT_HEALTH_BAR_HITBOX, bossPlayerID[i].x, bossPlayerID[i].y);
			healthbarHitbox.image_index = floor((BOSS_HEALTH-bossHealth)/12);
			healthbarHitbox.spr_dir = 1;
			healthbarHitbox.depth = bossPlayerID[i].depth-1;
